...
onMounted(() => {
  ...
  catalogoStore.cargarCamisasEstampadas()
})
</script>
    ...
    <div class="bg-white/60 backdrop-blur-sm border-b border-[#005F7F]/5 mb-8">
      <div class="container mx-auto px-6 py-8 max-w-[1600px]">
        <nav class="flex text-xs font-bold uppercase tracking-wider mb-3">
          ...
          <span class="text-[#002B42]">Camisas Estampadas</span>
        </nav>
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p class="text-sm text-[#005F7F]">
              Mostrando <span class="font-bold text-[#002B42]">{{ catalogoStore.camisasEstampadas.length }}</span>
              de {{ catalogoStore.camisasEstampadasRaw.length }} estilos únicos
            </p>
          </div>
    ...
    <main class="container mx-auto px-4 sm:px-6 pb-20 max-w-[1600px]">
      <div class="flex flex-col lg:flex-row gap-8">
        ...
        <div class="flex-1 order-2 lg:order-2">
          ...
          <!-- Sin resultados -->
          <div v-else-if="catalogoStore.camisasEstampadas.length === 0" class="text-center py-20">
          ...
          <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-12">
            <CamisaCard
              v-for="camisa in catalogoStore.camisasEstampadas"
              :key="camisa.id"
              :camisa="camisa"
            />
          </div>
